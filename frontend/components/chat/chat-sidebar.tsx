    ...
    >
      ...
      <div className="p-4 border-b border-violet-100 flex items-center justify-between">
        ...
        >
          ...
          <span className="text-lg font-bold text-black">Patent AI</span>
        </Link>
      </div>
      ...
      <button
        onClick={onNewChat}
        className="m-4 px-4 py-2 rounded-lg border border-violet-100 bg-white hover:bg-violet-100 transition-colors flex items-center justify-center gap-2 text-sm font-medium text-violet-900 cursor-pointer"
      >
        ...
        {threads.map((thread) => (
          ...
          >
            ...
            <div className="text-xs text-violet-400">
              {format(new Date(thread.created_at), "M월 d일 a h:mm", {
                locale: ko,
              })}
            </div>
          </div>
      ...
      <div className="p-4 border-t border-violet-200 space-y-2">
        ...
        <Link href="/#pricing">
          <Button
            variant="outline"
            className="w-full bg-white border-violet-200 text-violet-700 hover:bg-violet-50"
            size="sm"
          >
